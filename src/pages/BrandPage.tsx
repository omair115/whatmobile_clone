...
export function BrandPage() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    // First get brand name if possible, or just use slug
    setBrandName(brandSlug.charAt(0).toUpperCase() + brandSlug.slice(1));

    fetch(`/api/mobiles?brand=${brandSlug}`)
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] pb-12">
      <SEO 
        title={`${brandName} Mobile Prices in Pakistan - Latest ${brandName} Phones`} 
        description={`Check out the latest ${brandName} mobile phone prices and specifications in Pakistan. Find the best ${brandName} phone for your budget.`}
      />
      ...
      <div className="bg-[#1a3a5a] py-4 border-b border-[#0d2640]">
        <div className="container mx-auto px-4 flex items-center gap-2 text-white">
          ...
          <span className="text-sm font-bold uppercase tracking-wider">{brandName} Mobiles</span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
          <div className="bg-[#f8f9fa] border-b px-6 py-4">
            <h1 className="text-xl font-bold text-[#1a3a5a] uppercase tracking-tight">
              {brandName} Mobile Phone Prices in Pakistan
            </h1>
            <p className="text-xs text-muted-foreground mt-1">
              Showing {mobiles.length} {brandName} smartphones
            </p>
          </div>
          ...
          <div className="p-6">
            {isLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-muted-foreground font-medium">No mobiles found for this brand yet.</p>
              </div>
            )}
          </div>
        </div>
